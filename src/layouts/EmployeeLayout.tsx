...
// Give Clock In/Out its own distinct icon
import { Timer } from "lucide-react";

const ICON_OVERRIDE: Record<string, React.ElementType> = {
  "/clock": Timer,
};
  ...
  return (
    <div className="min-h-screen bg-slate-100">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 h-14 bg-slate-900 border-b border-slate-700/60 shadow-lg flex items-center gap-2 px-4">
        ...
        {/* Brand */}
        <NavLink to="/dashboard" className="flex items-center gap-2.5 shrink-0 mr-3">
        ...
        <nav className="hidden lg:flex items-center gap-0.5 flex-1 min-w-0 overflow-x-auto">
          {NAV_ITEMS.map(({ to, label, Icon }) => {
            const ResolvedIcon = ICON_OVERRIDE[to] ?? Icon;
            return (
              <NavLink key={to} to={to} className={linkClass}>
                <ResolvedIcon className="h-3.5 w-3.5 shrink-0" />
                {label}
              </NavLink>
            );
          })}
